...
export default function UserManagementPage() {
  ...
  const filtrados = useMemo(() => {
    ...
    return usersList.filter((u) => {
      ...
      const matchStatus =
        statusFilter === 'all' || (statusFilter === 'active' ? u.activo : !u.activo)
  ...
  const handleToggleActivo = (u) => {
    ...
    setNotice(
      t(
        u.activo ? 'admin.users.deactivatedSuccess' : 'admin.users.activatedSuccess',
  ...
  const handleEliminarUsuario = (u) => {
    try {
      ...
      setNotice(t('admin.users.deletedSuccess', `Usuario ${u.nombre} eliminado.`))
      ...
      if (err.message === 'hasActiveReservations') {
        ...
        setErrorModal(t('admin.users.deleteError', 'Error al eliminar usuario.'))
      }
    }
  }
      ...
      <main className="management-main" style={{ padding: '24px 32px' }}>
        <div className="cities-container" style={{ maxWidth: '100%' }}>
          ...
          <section className="cities-card">
            ...
            <div className="cities-summary">
              <strong>{filtrados.length}</strong> {t('admin.usersFound', 'usuarios registrados')}
            </div>
                  ...
                  <tbody>
                    {filtrados.map((u) => (
                      <tr key={u.id}>
                        ...
                        <td>
                          <span className={`doc-status-badge ${u.documentosEstado}`}>
                            {u.documentosEstado === 'aprobado' && <FaCheckCircle />}
                            {u.documentosEstado === 'pendiente' && <FaExclamationTriangle />}
                            {u.documentosEstado === 'rechazado' && <FaTimesCircle />}
                            {u.documentosEstado}
                          </span>
                        </td>
        ...
        {modalDocumentos && (
          <div
            ...
            onMouseDown={(e) => e.target === e.currentTarget && setModalDocumentos(null)}
          >
            <section className="cities-modal user-doc-modal">
              ...
              <label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--city-muted)' }}>
            ...
            <section className="cities-modal">
              ...
              {modalEliminar.reservasActivas > 0 && (
                ...
                >
                  ...
                  No es posible eliminar este usuario porque tiene {modalEliminar.reservasActivas} reserva(s)
                  activa(s).
                </div>
              )}
