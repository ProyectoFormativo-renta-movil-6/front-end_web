...
const VEHICULOS_MOCK = [
  {
    ...
    calificacion: 4.8, disponible: true,
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.52)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 20, padding: 28, width: '100%', maxWidth: 420, boxShadow: '0 24px 64px rgba(0,0,0,0.18)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
          ...
          <button onClick={onCerrar} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              ...
              <button key={op.val} onClick={() => s('tipoKm', op.val)} style={{
                ...
                border: `2px solid ${form.tipoKm === op.val ? '#1e3a8a' : '#e2e8f0'}`,
                background: form.tipoKm === op.val ? '#eff6ff' : '#fff',
    ...
    <div style={{ display: 'flex', gap: 28, alignItems: 'flex-start' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: 28, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          {/* ✅ IMAGEN CORREGIDA */}
          <div style={{ background: 'linear-gradient(135deg,#eff6ff,#dbeafe)', height: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
            {vehiculo.imagenes?.[0]
              ? <img src={vehiculo.imagenes[0]} alt={vehiculo.nombre} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : <svg width="68" height="68" fill="none" stroke="#1e3a8a" strokeWidth="1.2" viewBox="0 0 24 24" style={{ opacity: 0.3 }}>
          ...
          <div style={{ padding: '18px 22px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10 }}>
              <div>
                <span style={{ fontSize: 11, fontWeight: 700, color: '#1e3a8a', background: '#eff6ff', padding: '3px 9px', borderRadius: 6, marginBottom: 6, display: 'inline-block' }}>{vehiculo.categoria}</span>
              ...
              <div key={idx} style={{ borderRadius: 20, border: `2px solid ${sel ? '#1e3a8a' : '#e2e8f0'}`, background: sel ? '#f0f6ff' : '#fff', boxShadow: sel ? '0 4px 20px rgba(30,58,138,0.12)' : '0 2px 8px rgba(0,0,0,0.05)', overflow: 'hidden', transition: 'all 200ms' }}>
                <div style={{ padding: '18px 22px 0' }}>
                  <div style={{ display: 'flex', justifyContent: 'center', gap: 4, marginBottom: 8 }}>
                    {plan.icono.map((ic, i) => <span key={i} style={{ fontSize: 15 }}>{ic}</span>)}
                  </div>
                  <h4 style={{ fontSize: 17, fontWeight: 900, color: '#1e3a8a', textAlign: 'center', margin: '0 0 4px' }}>{plan.nombre}</h4>
                  <p style={{ fontSize: 18, fontWeight: 900, color: '#059669', textAlign: 'center', margin: '0 0 14px' }}>
                    COP {plan.precio.toLocaleString('es-CO')},00 <span style={{ fontSize: 12, fontWeight: 500, color: '#64748b' }}>/ por día</span>
                  </p>
                ...
                <div style={{ padding: '0 22px' }}>
                  {plan.items.map((item, i) => (
                    <div key={i} style={{ display: 'flex', gap: 9, alignItems: 'flex-start', marginBottom: 9 }}>
                      ...
                      <span style={{ fontSize: 12, color: item.tipo === 'x' ? '#94a3b8' : '#334155', lineHeight: 1.5, textDecoration: item.tipo === 'x' ? 'line-through' : 'none' }}>
                        {item.texto}
                      </span>
                    </div>
                  ))}
                ...
                <div style={{ padding: '18px 22px' }}>
                  <button onClick={() => onSeleccionar(idx)} style={{
                    width: '100%', padding: 12, borderRadius: 12, fontWeight: 800, fontSize: 13,
                    cursor: 'pointer', transition: 'all 200ms',
                    background: sel ? 'linear-gradient(90deg,#1e3a8a,#2563eb)' : idx === 1 ? 'transparent' : '#e2e8f0',
                    color: sel ? '#fff' : idx === 1 ? '#1e3a8a' : '#64748b',
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
                    <div style={{
                      ...
                      background: completado ? '#1e3a8a' : activo ? 'linear-gradient(135deg,#1e3a8a,#2563eb)' : '#e2e8f0',
